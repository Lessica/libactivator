...
- 阶段 4 当前已完成 hardware button 主路径和 status bar gestures：Volume/Menu/Sleep/Ringer 相关 HID 事件已完成真机验收，`LATStatusBarEventSource` 已实现并完成 stable 逻辑覆盖与真机验收。Top slide / edge gesture family 已接入 `_UISystemGestureWindow sendEvent:`，纯分类器 24 组真机验证已通过并覆盖 SpringBoard、LockScreen、Application 三类 mode；dispatch-no-intercept 路径已通过真机验收，分类成功后提交对应 `LAEvent`，仍不拦截原始系统手势。本阶段继续向 drag-along screen-side、multitouch、SpringBoard/icon 和 lock screen gesture family 切片推进。
...
1. Drag-along screen-side：Top slide / edge gesture 的 slide-in / two-finger-slide-in dispatch-no-intercept 链路已验收，下一步单独确认 `libactivator.drag-along.*` 的现代语义、触发阈值和是否可继续复用 `_UISystemGestureWindow sendEvent:` 的 `UIEvent` snapshot。
