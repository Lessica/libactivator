...
| 未完成 event family | 1.9.13 event 中仍有 13 个未完成验收 | 按 icon flick、lock screen clock、car/watch/clamshell 分组推进。 |
...
2. Lock screen clock gesture family：clock double tap、tap hold、swipe left/right/down 共 5 个 event。iOS 15 RootHide 与 iOS 16 rootless 的视图结构 probe、交互识别和 production 挂载复核已完成，待 owner 完成 assignment 端到端验收。
3. Car / watch / smart cover：依赖外设、设备能力或私有服务，先保留 metadata，不用 metadata presence 推断可用性。
