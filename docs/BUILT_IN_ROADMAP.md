...
| 未实现 event family | 1.9.13 event 中仍有 20 个未实现 | 按 icon flick、lock screen clock、headset button、gesture bar、scheduled、car/watch/clamshell 分组推进。 |
...
4. Gesture bar double tap：与 home indicator / gesture bar 设备能力和 iOS 版本强相关，需要按设备 probe。
5. Scheduled sunrise/sunset：需要定位旧实现语义和现代定位/日出日落调度来源。
6. Car / watch / smart cover：依赖外设、设备能力或私有服务，先保留 metadata，不用 metadata presence 推断可用性。
