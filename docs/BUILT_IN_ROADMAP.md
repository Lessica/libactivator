...
- Settings UI 不承担 runtime 行为；它只负责配置、展示，并调用 Public API 或明确属于产品配置面的私有 facade/IPC。
...
| 未完成 static event family | 1.9.13 bundled catalog 中仍有 8 个可用 name 未完成验收 | 先推进 icon flick，再处理 watch/clamshell；旧资源以版本 9999 禁用的两个 car event 单独保留。 |
| Dynamic event definitions | 1.9.13 provider inventory 已补齐；Network 垂直切片、production 跨进程配置面和 DEBUG CLI 临时前端已有，但尚未真机端到端验收 | 先完成 Network per-SSID 的 create/assignment/exact-fallback/remove 手工验收，再推进 Icon flick 的按 App exact definitions 和其余 providers。 |
| Dynamic listener catalog | 1.9.13 runtime catalog 欠账已补齐 | 按 application shortcuts、menu、contact、Flipswitch、ringtone、preset/configurable actions 等 family 分别决策和实现。 |
| Settings UI 与 menu | 尚未进入 runtime 主线 | 实现 `libactivatorsettings.dylib`、assignments/profile/blacklist UI、dynamic provider creation UI、menu editor 和 menu listener runtime provider。 |
...
| dynamic event definitions | `ActivatorTweak.dylib` 中的 `LATEventDefinitionRegistry` 与 family providers，以及 `libactivator.dylib` 中的 production manager facade/IPC | 显式管理 provider catalog、concrete event ownership、generation、property-list-safe generic create/config/remove 和持久化；Settings UI 与 DEBUG CLI 复用同一 generation-bound 产品配置面。Network 是首个接入 family，尚待真机手工验收。 |
...
| CLI | `subprojects/cli` 的 `/usr/bin/activator` | Release 命令面是生产兼容工具，使用 Public API 和生产 IPC，不依赖 testing IPC，也不是 test runner。Owner 明确要求的 `activator debug` 手工诊断前端用 `DEBUG` 隔离；其 `event-definitions` 子命令复用 production definition facade/IPC。 |

## Listeners/actions 剩余工作

当前不再按 1.9.13 的 117 项静态资源散列推进。该数量不包含旧版运行时生成的 listener catalog；完整动态欠账以 `BUILT_IN_ACTION_TRACKER.md` 为准。
...
6. 动态 listener catalog：逐 family 恢复 application shortcuts、menu、contact actions、Flipswitch、ringtones、brightness/volume presets、profiles、configurable speech/assistant/message/command/pause、PreferenceLoader 与 Shortcuts workflows；不能把静态 metadata 审计收口当作这些 family 已完成。

## Events 剩余工作

优先级按实现收益和可验证性排序：

1. Network per-SSID：按 `TESTING.md` 使用 DEBUG CLI 完成“catalog/create → assignment → SSID 变化 → exact handled/base fallback → remove 清理”真机端到端验收，并确认 generation 刷新与原 assignment 恢复。
2. Icon flick gesture family：接入 `icon.flick.*` 四个 base event 时，同步实现 `icon.flick.*.<bundle>` 的 provider、持久化、exact-first/fallback dispatch 与配置验收；只完成 base recognizer 不算 family 收口。
3. 其他 application-specific events：补齐 application launch、icon hold、icon 3D Touch 与 icon double tap 的按 App exact definitions。
4. 其他 user-created event providers：Mail、Notification、custom Scheduled、Battery thresholds。
5. Runtime-discovered event catalog：Bluetooth per-device 与 Touch ID per-finger；先确认现代系统是否仍提供稳定 identifier 和可用信号。
6. Watch / smart cover：依赖外设、设备能力或私有服务，先保留 metadata。旧资源以 `CoreFoundationVersion = 9999` 禁用的两个 car event 单独跟踪，不计入 8 个可用静态欠账。
...
- Dynamic provider catalog/create/remove 的 Settings creation UI；production typed manager、`LAActivator` facade 和 generation-bound IPC 已打通，Settings host 尚未消费这一产品配置面。Network per-SSID 已有 DEBUG CLI 临时前端，但真机端到端验收尚未执行。
...
边界：Settings UI 不实现 event acquisition，也不直接拥有 SpringBoard runtime state；它通过 Public API 与明确的 product configuration facade/IPC 操作 SpringBoard authoritative backend。

## CLI 遗留边界

CLI 的 release 命令面是 production compatibility tool，不是测试入口；Owner 明确要求的临时手工诊断前端可以用 `DEBUG` 完整隔离。
...
- `activator debug event-definitions` 只在 DEBUG image 中提供 catalog/create/remove 手工入口，但其调用的 typed manager、`LAActivator` facade 和 generation-bound IPC 是未来 Settings UI 复用的 production 产品配置面，不能随 CLI 的 `DEBUG` 条件裁掉。
