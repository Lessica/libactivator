...
| Built-in listeners/actions | No built-in listener/action implementation is registered yet. | Per-listener/action modern equivalent and package/resource layout; follow legacy `master` resource semantics because old third-party packages are unlikely to be available. | Listener/action priority and implementation notes. |
