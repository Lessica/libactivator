...
- 如果某个私有 SPI 值已经可以合理断定原始类型，就必须用该原始类型表达，不要用 `id` 指代；容器内元素也要用精确泛型，例如 `NSArray<AXElement *> *`、`NSArray<SBApplication *> *`，而不是裸 `NSArray *` 或 `NSArray<id> *`。只有 property-list、metadata、notification token、弱探测得到的 class object 或确实无法静态断定类型的动态边界，才保留 `id`。
...
- 不要在 ObjC 实现的同一个源码文件中声明本实现自己的私有方法；如果实现顺序已经能让编译器正常解析，就直接实现方法。Class extension 只用于属性、实例变量、协议采纳或确实需要暴露给同文件其他类型的声明。
- 含有 ObjC 类 `@implementation` 的实现文件中，类相关逻辑禁止写成 C helper；应使用 ObjC class method、instance method、局部类或已有 service 表达。纯 C callback、CaptainHook/Logos hook glue、C SPI trampoline 这类确实由 C ABI 要求的入口可以保留，但不能拿来承载普通类逻辑。
...
- `_accessibilityFrontMostApplication` / tweak-side raw foreground display identifier 不能在业务代码中直接当作“当前前台 App”。它只有在 runtime 已确认当前 mode 为 `application` 时才可信；锁屏、熄屏、CoverSheet 或其他 SpringBoard UI 覆盖时，该值可能只是下层 App。业务代码应读取 `displayIdentifierForCurrentApplication` 这类已按 runtime mode 过滤后的值，或者先显式判断非锁屏 / 非 SpringBoard UI 前提。
