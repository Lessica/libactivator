...
| Event Source module/registry | `run` | own-image module discovery、identifier/依赖/priority 稳定排序、capability gate、module result/binding、窄协议注入、typed ingress、source identifier/producer catalog、确定性启动、幂等、invalidate、同 event 多 producer、producer reload、assignment-aware interest | 扫描 SpringBoard/系统 framework class、definition ownership、真实 HID/手势/外设信号、把 metadata presence 当作 producer |
...
- Event Source module loader 的 stable tests 必须覆盖：只发现明确 image 内采用私有 module protocol 的 class、production 一对一 family 的发现 class 同时采用 `LATEventSource`、重复或非法 module identifier、依赖缺失与循环、依赖拓扑以及 priority/identifier 的稳定顺序、capability 未满足时无部分注册、factory/result validation、source/provider 注册失败回滚、typed ingress/service resolution。测试应向 loader 注入明确 module class 集合；不得为了测试扫描 SpringBoard 全进程或让 fake module 通过 `+load` 自注册。
...
- 新增 Event Source 必须在 stable 中覆盖 module factory/result、窄协议依赖注入、source 自声明 catalog、start/invalidate 和 interest 失效；dispatch 测试必须使用显式 adapter/fake，不能允许 source 回退到 `LASharedActivator`。同一 event 的多 producer 是合法关系，重复 source identifier 才应拒绝。
- Module-declared dynamic definition binding 必须复用 Dynamic provider 的 atomic diff/rollback suite，继续覆盖 definition 先于新增 mapping 可见、移除 mapping 先于 definition 注销、delegate reentrancy/postflight、失败后 provider/source/registry/persistence 全状态恢复；模块化不能削弱既有事务语义。
...
- 静态检查应覆盖：无 Logos、无 direct XPC、无 `CFMessagePort`、无不必要 `libSandy`、无 `ROOT_PATH` 宏族、无用户 App 注入 filter、无直接 `objc_msgSend`、新增代码/注释/日志无中文；`subprojects/tweak/events` 的 concrete sources 无 `LASharedActivator`；module discovery 无 `objc_getClassList` / `objc_copyClassList` 等全进程扫描，只允许 `class_getImageName` + `objc_copyClassNamesForImage` 的明确自有 image 路径；无 `+load` / constructor source 自注册；`LATBuiltInRegistry` 不 import 或逐项构造具体 Event Source/provider family。
