...
| --- | --- | --- |
| Event delivery | SpringBoard dispatch engine exists for event, abort, preview, deactivate, listener compatibility filtering, blacklist filtering, and handled-state propagation. Non-SpringBoard dispatch forwards to SpringBoard through IPC. | Implement built-in event sources and modern runtime state providers separately. |
...
| Runtime mode and foreground app state | `LAActivatorRuntimeStateProvider` centralizes placeholder values: `currentEventMode` is process-role based, `currentEventModeUnderneathLockScreen` is a fixed fallback, and `displayIdentifierForCurrentApplication` returns the current process bundle identifier. | Implement SpringBoard runtime state using the researched foreground app, lock screen, home screen, and screen-state references. |
| Unlock support | `supportsUnlockingDeviceToSendEvents` returns `NO`. | Design a private SpringBoard unlock service from the researched `SBLockScreenManager` path and verify no-passcode/passcode/biometric behavior before enabling. |
