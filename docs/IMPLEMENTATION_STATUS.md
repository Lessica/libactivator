...
| Built-in resource catalog | 61 event metadata bundles, 58 listener/action metadata bundles, and English / Simplified Chinese support localizations are staged under `/Library/Activator`. `libactivator.twitter.compose-tweet` is intentionally excluded; `libactivator.system.safemode` metadata is retained. | Metadata presence does not imply event acquisition or action implementation. |
...
| Built-in resource catalog | ✅ | Capability gated | Layout resources | Built-in event capability inventory, built-in listener/action inventory | Static event and listener/action metadata plus English and Simplified Chinese localization resources are staged without registering built-in action listeners. |
...
| Built-in listener/action implementation | Not started | Capability gated | Runtime adapters | Built-in listener/action inventory | Each built-in listener/action has a modern iOS capability assessment and validation result before registration. |
