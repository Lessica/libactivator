...
1.9.13 static listener/action 资源共 117 个。当前 staged static listener/action resource 共 118 个，其中 112 个来自 1.9.13，6 个是 2.x additive name。当前已实现 runtime static listener name 共 113 个，其中 107 个来自 1.9.13，6 个是 2.x additive name。

阶段 3 后续收口不再按原始 117 项散列推进；当前需要关注的是：staged 资源中仍未注册 runtime listener 的 5 个 name、已从 staged 移除且不恢复的 5 个旧 name，以及单独暂缓的 2.x additive `soft-reboot`。
