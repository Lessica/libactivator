...
| `receives-raw-events` | 1 | 让 listener 接收原始 event 以实现旧 `back` action 这类依赖当前 App/UIKit 上下文的行为。 | `out-of-scope-gap`：唯一使用者是 `libactivator.system.back`；当前项目不注入用户 App，不实现 old back raw-event 语义。 |
...
1.9.13 static listener/action 资源共 117 个。当前 staged static listener/action resource 共 118 个，其中 112 个来自 1.9.13，6 个是 2.x additive name。当前已实现 runtime static listener name 共 112 个，其中 106 个来自 1.9.13，6 个是 2.x additive name。

阶段 3 后续收口不再按原始 117 项散列推进；当前需要关注的是：staged 资源中仍未注册 runtime listener 的 6 个 name、已从 staged 移除且不恢复的 5 个旧 name，以及单独暂缓的 2.x additive `soft-reboot`。
...
| App 注入依赖 | `libactivator.system.back` | `out-of-scope` | 1.9.13 旧 `back` raw-event 语义依赖 `com.apple.UIKit` filter 注入用户 App 进程；本项目不注入用户 App，因此只保留资源/逆向记录，不实现。`libactivator.system.local-back` 已改用现代 Voice Control 的 `CACSpokenCommand goBack` 恢复。 |
