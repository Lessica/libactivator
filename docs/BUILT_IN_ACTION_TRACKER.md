...
| --- | ---: | ---: | --- |
| Events | 121 | 123 | 2 个 2.x additive now-playing 状态事件已加入；1.9.13 event 中仍有 19 个 runtime source 未实现。 |
...
1.9.13 event 资源共 121 个。当前未实现的 1.9.13 event name 共 19 个，按 family 归类如下。
...
Volume HUD tap 已通过独立 `LATVolumeHUDTapEventSource` 接入 `SBElasticVolumeViewController -viewDidLoad`，状态为 `implemented`，并从未完成计数移除。1.9.13 的 iOS 13 路径会在 `_sliderContainerView` 上无条件安装单击 recognizer，不使用 assignment interest gate；现代实现同样采用 always-on policy，并把 recognizer 配置为不取消或延迟 HUD 原触摸。真机 probe、安装后端到端手势验证和 owner 验收均已完成。

Gesture bar double tap 已通过独立 `LATGestureBarEventSource` 接入 `SBHomeGrabberRevealGesturesManager -init`，状态为 `implemented`，并从未完成计数移除。旧 master 尚无该事件；1.9.13 与 iOS 15 probe 均确认应复用 `_revealDoubleTapRecognizer`，只在 `Ended` 且触点位于参考 view 底部 `30pt` 内时 dispatch。Source 采用 always-on policy，但其统一 initializer 只有在 `fake-home-button` metadata gate 后仍存在 `libactivator.gesture-bar.double-tap` definition 时才返回实例，因此实体 Home 键设备不会注册该 source。现代 recognizer 对象链与真实双击已经完成 probe；安装后的端到端 event 验收仍待 owner 完成。
...
| Volume HUD tap | `libactivator.volume.display-tap` | `implemented` | 已作为独立 HUD/UI source 接入 `SBElasticVolumeViewController`，并完成端到端手势验收。 |
| Gesture bar | `libactivator.gesture-bar.double-tap` | `implemented` | 复用系统 `_revealDoubleTapRecognizer`，只在 `fake-home-button` 设备注册 source；待安装后端到端 event 验收。 |
