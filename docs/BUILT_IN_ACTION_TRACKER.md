...
1.9.13 static listener/action 资源共 117 个。当前已实现 static listener name 共 100 个，其中 94 个来自 1.9.13，6 个是 2.x additive name。未实现的 1.9.13 listener/action name 共 23 个，按处理状态归类如下；2.x additive 暂缓项单独列出。
...
| Camera shutter | `libactivator.camera.invoke-shutter` | `blocked` | 1.9.13 语义会先尝试相机快门；失败时打开 `com.apple.camera` 并等待 `libactivator.camera.ready` 后重试。现代实现需继续确认锁屏相机、前台相机和失败路径，不在本轮注册。 |
