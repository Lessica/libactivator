...
- Volume HUD tap 旧 master 为旧 `VolumeControl` / `SBVolumeHUDView` 创建透明顶层 `LAVolumeTapWindow`，只有当前 mode 已分配 listener 才显示覆盖窗口；窗口 `touchesBegan:` 立即发送 `libactivator.volume.display-tap`，event handled 时再隐藏旧 Volume HUD。1.9.13 的 iOS 13 路径已经改为 hook `SBElasticVolumeViewController -viewDidLoad`，从 `_sliderContainerView` 取出容器并无条件添加 `UITapGestureRecognizer`；新增 handler 仅在 recognizer state 为 `Ended` 时按当前 mode dispatch，不查询 assignment，也不根据 handled 隐藏 HUD。现代 iOS 真机 probe 确认该 class 与继承 ivar 仍存在，容器使用全屏 `SBFTouchPassThroughView`，实际 tap 只命中 HUD 子视图，普通点击触摸完整到达 ended，既有 pan / long-press recognizer 随后失败。当前 `LATVolumeHUDTapEventSource` 因此采用 always-on policy，并把新增 recognizer 配置为不取消或延迟原 HUD 触摸；这是在保持 1.9.13 安装时机与 dispatch 语义的同时，避免 acquisition 改变现代 slider 交互的明确差异。
