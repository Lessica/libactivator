...
- Definition provider 与 acquisition source 不得互相持有；`LATBuiltInRegistry` 作为 composition root 显式连接两层。Provider unregister、source unregister 及任一 registry invalidate 都不能隐式决定另一层对象的生命周期。
