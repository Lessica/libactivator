...
#define kLATStatusBarEventSourceMainQueueReason @"LATStatusBarEventSource must only be used on the main thread"
...
- (void)start {
    NSAssert(NSThread.isMainThread, kLATStatusBarEventSourceMainQueueReason);
    if (self.started) {
        return;
    }
    self.started = YES;
