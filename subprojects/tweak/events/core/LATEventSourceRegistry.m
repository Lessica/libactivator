...
- (BOOL)registerEventSource:(id<LATEventSource>)eventSource {
    ...
    id declaredEventNames = eventSource.eventNames;
    if (![declaredEventNames isKindOfClass:NSSet.class]) {
        HBLogWarn(@"Skipping event source %@ because its event name catalog is invalid", identifier);
        return NO;
    }
    NSSet<NSString *> *eventNamesSnapshot = [self normalizedEventNames:declaredEventNames];
    if (eventNamesSnapshot.count != [declaredEventNames count]) {
        HBLogWarn(@"Skipping event source %@ because its event name catalog contains invalid values", identifier);
        return NO;
    }
...
- (BOOL)reloadEventNamesForEventSource:(id<LATEventSource>)eventSource {
    ...
    id declaredEventNames = eventSource.eventNames;
    if (![declaredEventNames isKindOfClass:NSSet.class]) {
        HBLogWarn(@"Keeping the previous event names for %@ because the reloaded catalog is invalid",
                  [self.identifiersByEventSource objectForKey:eventSource]);
        return NO;
    }
    NSSet<NSString *> *eventNamesSnapshot = [self normalizedEventNames:declaredEventNames];
    if (eventNamesSnapshot.count != [declaredEventNames count]) {
        HBLogWarn(@"Keeping the previous event names for %@ because the reloaded catalog contains invalid values",
                  [self.identifiersByEventSource objectForKey:eventSource]);
        return NO;
    }
    NSSet<NSString *> *interestEventNamesSnapshot =
