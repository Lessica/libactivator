...
- (BOOL)eventWithName:(NSString *)eventName isCompatibleWithMode:(NSString *)eventMode {
    id value = [LAResourceManager.sharedManager eventInfoDictionaryForName:eventName][@"compatible-modes"];
...
- (BOOL)eventWithNameSupportsRemoval:(NSString *)eventName {
    id value = [LAResourceManager.sharedManager eventInfoDictionaryForName:eventName][@"supports-removal"];
    return [value respondsToSelector:@selector(boolValue)] ? [value boolValue] : NO;
}
...
- (NSString *)configurationViewControllerClassNameForEventWithName:(NSString *)eventName bundle:(NSBundle **)bundle {
    NSString *className =
        [LAResourceManager.sharedManager eventInfoDictionaryForName:eventName][@"settings-view-controller-class"];
    if (![className isKindOfClass:NSString.class] || className.length == 0) {
        className = [LAResourceManager.sharedManager eventInfoDictionaryForName:eventName][@"configuration"];
    }
    if (![className isKindOfClass:NSString.class] || className.length == 0) {
        return nil;
    }
