...
- (void)registerAvailableEventsWithActivator:(LAActivator *)activator {
    for (NSString *eventName in [LAResourceManager.sharedManager availableEventNames]) {
...
- (BOOL)eventWithNameIsHidden:(NSString *)eventName {
    return [[LAResourceManager.sharedManager eventInfoDictionaryForName:eventName][@"hidden"] boolValue];
}

- (BOOL)eventWithNameRequiresAssignment:(NSString *)eventName {
    id value = [LAResourceManager.sharedManager eventInfoDictionaryForName:eventName][@"requires-event"];
    return [value respondsToSelector:@selector(boolValue)] ? [value boolValue] : YES;
}

- (BOOL)eventWithName:(NSString *)eventName isCompatibleWithMode:(NSString *)eventMode {
    NSArray *modes = [LAResourceManager.sharedManager eventInfoDictionaryForName:eventName][@"compatible-modes"];
...
- (BOOL)eventWithNameSupportsRemoval:(NSString *)eventName {
    return [[LAResourceManager.sharedManager eventInfoDictionaryForName:eventName][@"supports-removal"] boolValue];
}

- (BOOL)eventWithNameSupportsUnlockingDeviceToSend:(NSString *)eventName {
    id value = [LAResourceManager.sharedManager eventInfoDictionaryForName:eventName][@"supports-unlocking-device"];
    return [value respondsToSelector:@selector(boolValue)] ? [value boolValue] : NO;
}

- (NSString *)assignmentWarningForEventWithName:(NSString *)eventName {
    id value = [LAResourceManager.sharedManager eventInfoDictionaryForName:eventName][@"assignment-warning"];
    return [value isKindOfClass:NSString.class] ? value : nil;
}

- (BOOL)eventWithNameIsUnprotected:(NSString *)eventName {
    id value = [LAResourceManager.sharedManager eventInfoDictionaryForName:eventName][@"is-unprotected"];
    return [value respondsToSelector:@selector(boolValue)] ? [value boolValue] : NO;
}

- (NSString *)configurationViewControllerClassNameForEventWithName:(NSString *)eventName bundle:(NSBundle **)bundle {
    NSDictionary *info = [LAResourceManager.sharedManager eventInfoDictionaryForName:eventName];
    ...
    if (className.length > 0 && bundle) {
        *bundle = [LAResourceManager.sharedManager configurationBundleForEventName:eventName];
    }
    return className;
}
